...
// Printing the pareto fronts
  ...
  while((nNbRemoved<=nNB_IND) && (nParetoFrontNb<5)){
    ...
    for (int i=0;i<nNB_IND;i++) // now decreasing dominated
      if (fA[i][2]==-1){ // I am a dominant !!!
        printf("Decreasing individuals dominated by (%f,%f)\n",fA[i][0],fA[i][1]);
        for(int j=0;j<nNB_IND;j++){
          if(nPareto[i][j]!=-1){
            int bStronglyDominated=1;
            for (int m=0;m<nNbObjectives;m++)
              if (fA[nPareto[i][j]][m]==fA[i][m]) bStronglyDominated=0;
            if (bStronglyDominated){
              if (fA[nPareto[i][j]][2]>0) fA[nPareto[i][j]][2]--;
              printf("   Decreased (%f,%f) now =%d\n",fA[nPareto[i][j]][0],fA[nPareto[i][j]][1],(int)fA[nPareto[i][j]][2]);
            }
          }
          else break;
        }
       fA[i][2]=-2; // not a dominant anymore
       nNbRemoved++;
      }
    nParetoFrontNb++;
  }
